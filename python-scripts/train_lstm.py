...
def align_matrix_and_sequences(matrix: pd.DataFrame, sequences: np.ndarray,
    ...
    assert not matrix_aligned["hadm_id"].isna().any(), "NaN hadm_id after align."
    assert len(matrix_aligned) == len(sequences_aligned), "Length mismatch."
    assert (matrix_aligned["hadm_id"].values == kept_ids).all(), "Order mismatch."
...
def preprocess_tabular(matrix: pd.DataFrame, feature_cols: List[str],
    ...
    # assertion 7: the three partitions exactly cover this cohort, no gap, no overlap
...
class SequenceDataset(Dataset):
    """
    Wraps sequences, optional tabular features, and labels.
    If X_tabular is None, __getitem__ returns (seq, y); else (seq, tab, y).
    """
...
class LSTMNet(nn.Module):
    ...
    def __init__(self, n_tabular: int = 0):
        ...
        self.lstm = nn.LSTM(input_size=EMBED_DIM, hidden_size=RNN_HIDDEN,
                            batch_first=True)
        head_in = RNN_HIDDEN + n_tabular
...
def run_experiment(cohort: str, condition: str) -> Dict:
    print(f"\n{'='*60}", flush=True)
    print(f"  LSTM | {cohort} | {condition}", flush=True)
...
def main():
    ...
    print(f"\n{'#'*60}", flush=True)
    print(f"# ALL LSTM EXPERIMENTS COMPLETE in {overall:.1f}s", flush=True)
    print(f"{'#'*60}", flush=True)

    df = pd.DataFrame(results)
    out_path = RESULTS / f"lstm_results{TAG}.csv"
    df.to_csv(out_path, index=False)
    print(f"\nSaved: {out_path}", flush=True)
    print(f"\nLSTM Results:", flush=True)
