...
def run_experiment(cohort, condition):
    ...
    return {'Model':'CNN-k23','Cohort':cohort,'Condition':condition, **mtr, 'Epochs':ep, 'Train_Time_s':el, 'split_id': SPLIT_ID[cohort]}
...
def main():
    ...
    experiments = [(c, cond) for c in COHORTS for cond in ("Sequence Only", "Sequence + Tabular")]
    ...
    df = pd.DataFrame(results); out = RESULTS/f"cnn_k23_results{TAG}.csv"; df.to_csv(out, index=False)
