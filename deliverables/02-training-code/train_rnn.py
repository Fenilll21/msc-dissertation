...
def align_matrix_and_sequences(matrix: pd.DataFrame, sequences: np.ndarray,
    ...
    """Align matrix and sequences to intersection of hadm_ids (same as CNN)."""
...
class VanillaRNN(nn.Module):
    def __init__(self, n_tabular: int = 0):
        ...
        self.rnn = nn.RNN(input_size=EMBED_DIM, hidden_size=RNN_HIDDEN,
...
def run_experiment(cohort: str, condition: str) -> Dict:
    print(f"\n{'='*60}", flush=True)
    print(f"  RNN | {cohort} | {condition}", flush=True)
...
def main():
    ...
    print(f"\n{'#'*60}", flush=True)
    print(f"# ALL RNN EXPERIMENTS COMPLETE in {overall:.1f}s", flush=True)
    print(f"{'#'*60}", flush=True)

    df = pd.DataFrame(results)
    out_path = RESULTS / f"rnn_results{TAG}.csv"
    df.to_csv(out_path, index=False)
    print(f"\nSaved: {out_path}", flush=True)
    print(f"\nRNN Results:", flush=True)
