...
def run_experiment(cohort: str, condition: str) -> Dict:
    print(f"\n{'='*60}", flush=True)
    print(f"  BiLSTM | {cohort} | {condition}", flush=True)
...
def main():
    ...
    print(f"\n{'#'*60}", flush=True)
    print(f"# ALL BiLSTM EXPERIMENTS COMPLETE in {overall:.1f}s", flush=True)
    print(f"{'#'*60}", flush=True)

    df = pd.DataFrame(results)
    out_path = RESULTS / f"bilstm_results{TAG}.csv"
    df.to_csv(out_path, index=False)
    print(f"\nSaved: {out_path}", flush=True)
    print(f"\nBiLSTM Results:", flush=True)
    print(df.to_string(index=False), flush=True)

def _smoke_test():
    """Shape/dtype sanity check: no training, just a forward pass."""
